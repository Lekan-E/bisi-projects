...
# sidebar dropdown shows friendly labels ("Cluster 3 (Safest)"); map the choice
# back to the raw cluster id used everywhere else in the data
with st.sidebar:
    st.header("Cluster")
...
# headline stats for the selected cluster
...
# sum incidents per cluster, then convert to a 0-100% share so clusters of different sizes compare fairly
...
# bars for the share, plus a second text layer to print the "%" label past the bar's end
comp_bars = (
    alt.Chart(share_row)
    .mark_bar()
    .encode(
...
# z-score each feature city-wide, then average within the cluster —
# positive = above the city average, negative = below it
z = feature_zscores(df)
cluster_z = z.groupby("cluster")[cols].mean().loc[cid].sort_values(ascending=False)

# only show the most above-average and most below-average features, not all of them
top_n = 6
...
# red bars for above-average features, blue for below-average
z_chart = (
    alt.Chart(z_df)
    .mark_bar()
    .encode(
